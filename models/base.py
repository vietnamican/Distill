...
class ConvBatchNormActivation(Base):
    def __init__(self, *args, **kwargs):
        super().__init__()
        activation = None
        ...
        if activation is not None:
            self.cbr.add_module('activation', activation)
    ...
    def _release(self):
        if self.with_bn:
            ...
            activation = None
            if self.cbr.activation is not None:
                activation = self.cbr.activation
            if activation is not None:
                self.cbr = nn.Sequential()
                self.cbr.add_module('conv', conv)
                self.cbr.add_module('relu', activation)
            else:
                self.cbr = nn.Sequential()
                self.cbr.add_module('conv', conv)
...
def ConvBatchNormRelu(*args, **kwargs):
    activation= nn.ReLU(inplace=True)
    return ConvBatchNormActivation(*args, **kwargs, activation=activation)

def ConvBatchNormRelu6(*args, **kwargs):
